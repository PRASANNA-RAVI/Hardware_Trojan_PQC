...
extern const uint8_t ATTACKER_MASTER_KEY[32];
...
int crypto_kem_enc_attack(unsigned char *c, unsigned char *k, const unsigned char *pk)
{
  ...
  sha3_512(kr, buf, 64);               // kr[0:63] <-- Hash(buf[0:63]);
                                       // K^ <-- kr[0:31]
                                       // noiseseed (r) <-- kr[32:63];
  indcpa_kem_enc_attack(buf, kr + 32, pk, c); // buf[0:31] contains message; kr[32:63] contains randomness r;
...
int crypto_kem_dec(unsigned char *k, const unsigned char *c, const unsigned char *sk)
{
  ...
  // for(int re = 1; re < 32; re++)
  for(int re = 0; re < 31; re++)
  {
    if(buf[re] != ATTACKER_MASTER_KEY[re])
      trojan_active = 0;
  }
